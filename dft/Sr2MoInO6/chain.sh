#!/bin/bash
# Sr2MoInO6 pipeline: PBE+U relax (FM and AFM) -> pick lower-E0 magnetic order
...
for m in FM AFM; do
  ...
  ( cd "$d" && $MPI -np $NP --bind-to none --mca pml ob1 --mca btl self,sm "$VASP" > vasp.log 2>&1 )
...
( cd hse06 && $MPI -np $NP --bind-to none --mca pml ob1 --mca btl self,sm "$VASP" > vasp.log 2>&1 )
